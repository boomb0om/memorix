...
/**
 * Компонент блока вопроса с несколькими ответами
 */
          ...
          <ul style={{ listStyle: 'none', padding: 0 }}>
            {(block.options || []).map((opt, optIdx) => {
              ...
              return (
                <li 
                  key={optIdx} 
                  onClick={() => !checkedResult?.is_correct && onToggle(optIdx)}
                  style={{ 
                    padding: '8px 12px', 
                    marginBottom: '8px', 
          ...
          {!checkedResult?.is_correct && (
            <button
              ...
              style={{
                marginTop: '12px',
                padding: '8px 16px',
                background: (selectedAnswers && selectedAnswers.length > 0) ? '#6366f1' : '#9ca3af',
                color: 'white',
                border: 'none',
                borderRadius: '6px',
                cursor: (selectedAnswers && selectedAnswers.length > 0) ? 'pointer' : 'not-allowed',
                fontWeight: '500'
              }}
            >
              Проверить ответ
