...
class LessonCreate(BaseModel):
    ...
    position: int | None = Field(
        default=None,
        description="Позиция урока в курсе (None для автоматического назначения)",
        ge=-1,
    )
    name: str = Field(description="Название урока", min_length=1, max_length=200)
    description: str | None = Field(
        default=None, description="Описание урока", max_length=1000
    )
...
class LessonUpdate(BaseModel):
    ...
    name: str | None = Field(
        default=None, description="Название урока", min_length=1, max_length=200
    )
    description: str | None = Field(
        default=None, description="Описание урока", max_length=1000
    )
